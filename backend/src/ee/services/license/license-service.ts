...
import {
  ...
  UNLIMITED_ENTERPRISE_LICENSE_KEY
} from "./license-fns";
import {
  InstanceType,
  LicenseType,
...
export const licenseServiceFactory = ({
  ...
  const init = async () => {
    try {
      ...
      // Hardcoded key: unlock all enterprise features locally (no license-server call).
      if (licenseKeyConfig.isValid && licenseKeyConfig.licenseKey === UNLIMITED_ENTERPRISE_LICENSE_KEY) {
        onPremFeatures = getEnterpriseFeatures();
        instanceType = InstanceType.EnterpriseOnPremOffline;
        logger.info(`Instance type: ${InstanceType.EnterpriseOnPremOffline} (unlimited enterprise license)`);
        isValidLicense = true;
        return;
      }
